...
export const useUserProfileAndAircraftData = (selectedAirline: string, selectedAircraftType: string, initialAircraftTypeFromVatsim?: string, initialAircraftRegistrationFromBooking?: string, bookingId?: string) => {
  ...
  const [availableAirlines, setAvailableAirlines] = useState<Airline[]>([]); // New state for available airlines
    ...
    const fetchUserProfile = async () => {
      ...
      if (user) {
        const { data, error } = await supabase
          .from('profiles')
          .select('first_name, last_name, vatsim_ivao_id, rank, type_ratings, authorized_airlines') // Select new field
          .eq('id', user.id)
          .single();
  ...
  useEffect(() => {
    ...
    if (userProfile.authorized_airlines && userProfile.authorized_airlines.length > 0) {
      ...
      setAvailableAirlines(ALL_AIRLINES); // If no specific airlines are authorized, all are available
  ...
  return {
    ...
    availableAirlines, // Return availableAirlines
  };
};