...
interface Flight {
  ...
  user_profile: {
    ...
    is_staff: boolean;
    email: string | null;
    vatsim_ivao_id: string | null; // Added for consistency
  } | null;
}
...
export const useStaffDashboardData = () => {
  ...
  const fetchProfilesData = useCallback(async (userIds: string[]) => {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .select('id, display_name, is_staff, vatsim_ivao_id'); // Include vatsim_ivao_id
    if (error) {
      console.error('Error fetching profiles data for user IDs:', error);
      return {};
    }
    const profilesMap: { [key: string]: { display_name: string | null, is_staff: boolean, vatsim_ivao_id: string | null } } = {};
    data.forEach(profile => {
      profilesMap[profile.id] = { display_name: profile.display_name, is_staff: profile.is_staff, vatsim_ivao_id: profile.vatsim_ivao_id };
    });
    return profilesMap;
  }, []);
  ...
  const fetchAllFlights = useCallback(async () => {
    ...
    const flightsWithProfiles = data.map(flight => ({
      ...
      user_profile: {
        ...
        vatsim_ivao_id: userProfiles[flight.user_id]?.vatsim_ivao_id || null, // Added
      },
    }));
