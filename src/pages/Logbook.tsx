...
import { fetchVatsimPilotData } from '@/utils/vatsim';
...
const Logbook = () => {
  ...
  const fetchFlights = async () => {
    ...
    const selectString = "*,profiles(display_name,is_staff,vatsim_ivao_id)";
  ...
  return (
    <div className="container mx-auto p-4 pt-24">
      ...
      <div className="mb-8 text-center space-x-4">
        <Button onClick={handleLogVatsimFlight} className="px-6 py-3 text-lg" disabled={isLoggingVatsimFlight}>
          {isLoggingVatsimFlight ? 'Checking VATSIM...' : 'Log Active VATSIM Flight'}
        </Button>
        {hasSavedFlight && (
          <Button onClick={handleResumeFlight} className="px-6 py-3 text-lg" variant="secondary">
            Resume Saved Flight
          </Button>
        )}
      </div>
