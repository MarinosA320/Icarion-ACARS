...
const PlanFlight = () => {
  ...
  const [userTypeRatings, setUserTypeRatings] = useState<string[] | null>(null);
  const [departureAirport, setDepartureAirport] = useState('');
  const [arrivalAirport, setArrivalAirport] = useState('');
  const [selectedAirline, setSelectedAirline] = useState('Icarion Virtual');
  ...
  useEffect(() => {
    const fetchUserRankAndRatings = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        const { data, error } = await supabase
          .from('profiles')
          .select('rank, type_ratings')
          .eq('id', user.id)
          .single();
        if (error) {
          console.error('Error fetching user rank and type ratings:', error);
          showError('Error fetching user profile data.');
        } else if (data) {
          setUserRank(data.rank);
          setUserTypeRatings(data.type_ratings);
          console.log('PlanFlight: Fetched user rank:', data.rank, 'and type ratings:', data.type_ratings);
        }
      }
    };
    fetchUserRankAndRatings();
  }, []);
  ...
  useEffect(() => {
    ...
    if (currentAirline && userRank && userTypeRatings !== null) {
      const initialAircraftTypes = getFilteredAircraftTypes(currentAirline);
      console.log('PlanFlight: Filtered aircraft types for display:', initialAircraftTypes);
      ...
      console.log('PlanFlight: Not enough data to filter aircraft types. Current state: selectedAirline=', selectedAirline, 'userRank=', userRank, 'userTypeRatings=', userTypeRatings);
      setAircraftType('');
      setAircraftRegistration('');
    }
  }, [selectedAirline, userRank, userTypeRatings]);
  ...
  const getFilteredAircraftTypes = (airline: Airline) => {
    ...
    const filteredByRankAndTypeRating = availableAircraftTypes.filter(type => {
      ...
      const meetsRank = requiredRank ? hasRequiredRank(userRank, requiredRank) : true;
      const meetsTypeRating = hasTypeRating(userTypeRatings, type);
  ...
  const handleBookFlight = async () => {
    ...
    // Re-validate rank and type rating before booking
    const currentAirline = ALL_AIRLINES.find(a => a.name === selectedAirline);
    if (currentAirline) {
      const requiredRank = AIRCRAFT_MIN_RANKS[aircraftType];
      const meetsRank = requiredRank ? hasRequiredRank(userRank, requiredRank) : true;
      const meetsTypeRating = hasTypeRating(userTypeRatings, aircraftType);

      if (!meetsRank) {
        showError(`Your current rank (${userRank}) is not sufficient for the ${aircraftType}. Required: ${requiredRank}.`);
        return;
      }
      if (!meetsTypeRating) {
        showError(`You do not have the required type rating for the ${aircraftType} (${AIRCRAFT_FAMILIES[aircraftType] || 'N/A'} family).`);
        return;
      }
      ...
      if (currentAirline.bases.length > 0 && selectedAirline !== 'Aegean Airlines' && selectedAirline !== 'Ryanair') {
        const isDepartureAtBase = currentAirline.bases.includes(departureAirport.toUpperCase());
        const isArrivalAtBase = currentAirline.bases.includes(arrivalAirport.toUpperCase());
    ...
    if (error) {
      ...
      setDepartureAirport('');
      setArrivalAirport('');
      setSelectedAirline('Icarion Virtual');
      setAircraftType('');
      setAircraftRegistration('');
      setFlightNumber('');
      setEtd('');
      setEta('');
      localStorage.removeItem('planFlightForm');
    }
    setLoading(false);
  };
      ...
      <Card className="max-w-2xl mx-auto shadow-md rounded-lg">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <Label htmlFor="aircraftType">Aircraft Type (Your Rank: {userRank}, Ratings: {userTypeRatings?.join(', ') || 'None'})</Label>
              <Select value={aircraftType} onValueChange={handleAircraftTypeChange} disabled={!selectedAirline || !userRank || userTypeRatings === null}>
